...
public class movingevent : MonoBehaviour {
	...
	void Update () {
        if (Input.GetMouseButton(0))
        {
            ...
            {
                if(hit.transform.gameObject.tag=="charactor")
                {
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.tag == "fruit")
        {
            ...
            Instantiate(animation, new Vector3(collision.gameObject.transform.position.x, collision.gameObject.transform.position.y, -2), Quaternion.identity);
